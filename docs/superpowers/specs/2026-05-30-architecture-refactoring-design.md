...
- `.claude/rules/` 分层体系（8 个 .md 文件），基于 MSEC 四大原则
- 禁止项 + 模板代码 + 精确版本红线
...
```
.claude/rules/
├── 00-core.md              # 无 paths → 始终加载
├── 10-tech-stack.md        # 无 paths → 始终加载（版本红线全局适用）
├── 20-directory.md         # paths: ["src/**"]
├── 30-main.md              # paths: ["src/main/**"]
├── 31-renderer.md          # paths: ["src/renderer/**"]
├── 40-api.md               # paths: ["src/main/domains/**", "src/main/server/**"]
├── 50-testing.md           # paths: ["**/*.test.*", "**/*.spec.*", "**/__tests__/**"]
└── 60-security.md          # paths: ["src/main/**"]
```

### 4.3 各规则文件内容定义

**00-core.md** — 全局禁止项 + 必须项（无 paths，始终加载）：
...
**10-tech-stack.md** — 版本红线 + 禁止 API（无 paths，始终加载）：
...
2. **模板优于原则** — 30-main.md 和 31-renderer.md 各含可复制的完整模板，减少 AI 猜测空间
3. **瘦文件** — 每个 .md ≤ 50 行（不含模板），CLAUDE.md ≤ 30 行
...
| 路径 | 职责 |
|------|------|
| `.claude/rules/00-core.md` | 全局铁律 |
| `.claude/rules/10-tech-stack.md` | 技术栈版本红线 |
| `.claude/rules/20-directory.md` | 目录边界 |
| `.claude/rules/30-main.md` | 主进程 domain 模板 |
| `.claude/rules/31-renderer.md` | 渲染进程 feature 模板 |
| `.claude/rules/40-api.md` | Hono API 规范 |
