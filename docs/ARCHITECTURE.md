...
│   │   └── __tests__/
│   ├── domains/              # 业务层（7 domain，三件套：types/schema/service）
│   │   └── {provider,apikey,conversation,models,agent,logs,stats}/
│   │       └── __tests__/
...
7 个 domain，每个三件套（types/schema/service）：
...
48 个 `ipcMain.handle` + 4 个 `ipcMain.on`，按域拆分文件，全部经 `wrapIpcHandler` 包装（ZodError → `Invalid input: ...`，其他 → 日志 + `{error}`）。
...
**合计 48 文件、约 628 用例**（后端 547 + 前端 81）。
