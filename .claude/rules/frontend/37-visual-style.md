...
- 语义颜色（成功/错误/警告）：使用 `index.css` 中定义的 CSS 变量：
  ```css
  --success: #22c55e; --destructive: #ef4444; --warning: #eab308;
  ```
  引用方式：`text-[var(--success)]`、`bg-[var(--destructive)]` 等
  现有代码中的 `text-green-500`/`text-red-500`/`text-yellow-500` 允许保留，新代码必须使用 CSS 变量
