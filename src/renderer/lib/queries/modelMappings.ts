 ...
 *
 * TanStack Query 用法：
 * - useModelMappings: 列表查询，queryKey=['modelMappings', 'list']
 * - useModels: 模型列表查询，queryKey=['models', 'list']
 * - useCreateModelMapping: mutation 成功后自动 invalidate 'modelMappings' 缓存触发刷新
 * - useUpdateModelMapping: 同上
 * - useDeleteModelMapping: 同上
 *
 * 缓存策略：所有写操作（CUD）成功后 invalidate 'modelMappings'，下次读取时自动重新 fetch。
 */
...
export function useCreateModelMapping() {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['modelMappings', 'list'] }),
  })
}
